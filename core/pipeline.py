...
from response.containment import maybe_contain
...
class LivePacketStream:
    ...
    def subscribe(self) -> queue.Queue[str]:
        subscriber: queue.Queue[str] = queue.Queue(maxsize=LIVE_REPLAY_SIZE + 256)
        with self._lock:
...
def run(interface: InterfaceInfo) -> None:
    ...
    stored, alerted on (if severe enough), and possibly contained
            ...
            while True:
                ...
                for detector in DETECTORS:
                    ...
                    try:
                        ...
                        # dry-run only — never actually blocks traffic from
                        # this pipeline. See response/containment.py.
                        maybe_contain(finding, event.severity, live=False)
