...
def test_main_prints_interfaces_with_short_option(monkeypatch):
    interfaces = [object()]
    printed = []
    monkeypatch.setattr(launcher.sys, "argv", ["run.py", "-i"])
    monkeypatch.setattr(launcher, "list_interfaces", lambda: interfaces)
    monkeypatch.setattr(launcher, "print_interface_err", printed.append)
    monkeypatch.setattr(
        launcher,
        "run",
        lambda interface: (_ for _ in ()).throw(AssertionError("run should not start")),
    )

    assert launcher.main() == 0
    assert printed == [interfaces]
